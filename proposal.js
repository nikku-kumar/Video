...
const chemistry = [['🌸', 'Cute'], ['💕', 'Very Romantic'], ['🔥', 'Dangerously Romantic'], ['😈', 'Betu Decides']];
...
function welcome() {
  layout(`<section class="screen"><div class="card"><p class="kicker reveal">A little adventure from Nikku</p><h1 class="reveal reveal-late">Hey Betu <span class="accent">❤️</span></h1><p class="lead reveal reveal-later">I have something very important to ask you... 🥰</p><p class="lead reveal reveal-later"><strong>Betu, will you go on a date with me? ❤️🥺</strong></p><div class="actions reveal reveal-later"><button class="primary" data-yes>YES ❤️</button><div class="no-wrap"><button class="secondary no-button" data-no>NO 🙈</button><p class="no-message" data-no-message></p></div></div><p class="small-note reveal reveal-later">P.S. I promise good food and very bad jokes.</p></div></section>`);
...
function playfulNo() {
  ...
  const lines = ['Ehh Betu... are you sure? 🥺👉👈', 'Wait wait... I think you clicked the wrong button. 😂❤️', "Let's try that again, Betu... 😌", 'Betuuu... think about it one more time 🥺❤️', "Nikku is standing here with his best date proposal... 😭😂", 'One tiny YES could make someone’s whole day. 🥰', 'Okay okay... I’ll give you one last chance. 😌❤️'];
...
function foodPlan() {
  ...
  layout(`<section class="screen wide-screen"><div class="card"><p class="kicker">Chapter two: the menu</p><h2>What is your favourite food, Betu? 🍗❤️</h2><div class="food-grid expanded-food">${cards}</div><p class="lead food-result" data-food-result></p></div></section>`);
  document.querySelectorAll('[data-food]').forEach((choice) => choice.addEventListener('click', () => { selectedFood = choice.dataset.food; document.querySelectorAll('[data-food]').forEach((item) => item.classList.remove('selected')); choice.classList.add('selected'); document.querySelector('[data-food-result]').innerHTML = `Ooooh... good choice, Betu. 😋❤️<br>So we're having <strong>${selectedFood}</strong> on our date!<br>Nikku approves this decision. 😂❤️<br><button class="primary" data-plan-next>Plan the date day →</button>`; action('[data-plan-next]', () => { step = 4; render(); }); }));
}
...
function momentReaction() {
  const copy = { 'A Sweet Kiss': ['Hehehe... someone is getting brave today. 😳💋', 'A sweet kiss it is. Nikku is blushing already. ❤️', '💋'], 'A Long Hug': ['Okay... one very long hug coming right up. 🤗❤️', "Because sometimes words aren't enough... a hug says everything.", '🤗'], 'A Little More Romance': ['Ohhh Betu... now you’re making Nikku blush. 😳❤️', 'I like where this little story is going.', '❤️'] }[selectedMoment] || ['That sounds lovely. 🌙❤️', 'As long as we are together, the plan is perfect.', '🌙'];
...
function chemistryPage() {
  layout(`<section class="screen"><div class="card"><p class="kicker">The chemistry check</p><h2>Okay Betu... one last question. 👀</h2><p class="lead"><strong>How romantic should tonight be?</strong></p><div class="moment-grid chemistry-grid">${chemistry.map(([emoji, name]) => `<button class="choice" data-chemistry="${name}"><span>${emoji}</span>${name}</button>`).join('')}</div><p class="lead chemistry-result" data-chemistry-result></p></div></section>`);
  document.querySelectorAll('[data-chemistry]').forEach((choice) => choice.addEventListener('click', () => { const responses = { Cute: 'Okayyy... keeping it sweet. 🥰', 'Very Romantic': 'Now we’re talking. ❤️', 'Dangerously Romantic': 'Umm... Betu, should Nikku be nervous? 😳🔥', 'Betu Decides': 'Best answer. 😂❤️' }; document.querySelectorAll('[data-chemistry]').forEach((item) => item.classList.remove('selected')); choice.classList.add('selected'); document.querySelector('[data-chemistry-result]').innerHTML = `${responses[choice.dataset.chemistry]}<br><button class="primary" data-next>Read something from my heart ✨</button>`; action('[data-next]', () => { step = 8; render(); }); }));
}
function shayari() {
  const lines = ['Teri muskurahat ka koi jawaab nahi...', 'Tere jaisa khoobsurat koi khwaab nahi...', 'Tu saath ho toh har pal khaas lagta hai...', 'Betu, tera naam hi dil ke sabse paas lagta hai. ❤️'];
  layout(`<section class="screen poetry-screen"><div class="card"><p class="kicker">A few words from the heart</p><h2>For Betu, line by line ✨</h2><div class="poetry-lines">${lines.map((line) => `<p>${line}</p>`).join('')}</div><div class="letter-list"><div class="letter-line">Chaand ko dekha toh tera khayal aaya,<br>Teri hasi ne phir dil ko muskuraaya...</div><div class="letter-line">Sach kahun Betu... mujhe perfect date nahi chahiye.</div><div class="letter-line">Bas tere saath ek imperfect moment bhi perfect lagta hai. ❤️</div></div><button class="primary" data-next>One last surprise 🎁</button></div></section>`);
  action('[data-next]', () => { step = 9; render(); });
}
function photoPlaceholder() { const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 700 700"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#e7929e"/><stop offset=".52" stop-color="#c9a8d3"/><stop offset="1" stop-color="#f4ce98"/></linearGradient></defs><rect width="700" height="700" fill="url(#g)"/><circle cx="120" cy="130" r="60" fill="#fff" opacity=".25"/><circle cx="565" cy="545" r="145" fill="#fff" opacity=".18"/><text x="350" y="325" text-anchor="middle" fill="#fff" font-family="Georgia" font-size="48">Betu's photo</text><text x="350" y="380" text-anchor="middle" fill="#fff" opacity=".8" font-family="sans-serif" font-size="20">add images/surprise.jpg</text></svg>`; return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`; }
function finalIntro() {
  layout(`<section class="screen"><div class="card"><p class="kicker">The final chapter</p><h2>Betu...</h2><p class="lead">You've reached the final surprise. 👀</p><p class="lead"><strong>But I need you to promise me something...</strong></p><button class="primary" data-promise>What? 👀</button></div></section>`);
...
function finalReveal() {
  layout(`<section class="screen final-reveal"><div class="card"><div class="reveal-sparkles">✦ ✧ ✦</div><p class="kicker">For my favourite person</p><h2>SURPRISE, Betu 🎁</h2><figure class="photo-frame"><img src="./images/WhatsApp Image 2026-08-19 at 8.50.29 PM.jpeg" data-photo alt="Betu's surprise photo"><figcaption class="photo-caption">This is my favourite person. ❤️<br>My favourite smile.<br>My favourite Betu. 🥰</figcaption></figure><button class="primary" data-next>I Love You ❤️</button></div></section>`, 'final-reveal');
